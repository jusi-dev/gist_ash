defmodule GistAshWeb.GistLive.New do
  ...
  def handle_event("save", %{"form" => form_params}, socket) do
    current_user = socket.assigns.current_user || raise "No user in socket assigns"

    # Convert files map to list
    files_params = Map.get(form_params, "files", %{}) |> Map.values()

    # Prepare params with user_id
    params = %{
      "description" => form_params["description"],
      "public" => form_params["public"] == "true",
      "files" => files_params,
    ...
    case Ash.create(changeset, domain: GistAsh.Gists, actor: current_user) do
      {:ok, gist} ->
        {:noreply,
         socket
